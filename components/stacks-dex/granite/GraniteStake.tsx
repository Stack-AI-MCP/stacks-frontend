  ...
  return (
    <TransactionWrapper transactionData={transactionData} network={transaction.network === "mainnet" ? "mainnet" : "testnet"} buttonText="Sign & Stake" buttonGradient="from-slate-600 to-gray-600 hover:from-slate-700 hover:to-gray-700">
      <Card className="w-full bg-gradient-to-br from-slate-500/10 to-gray-500/10 border-slate-500/20">
        <CardHeader><div className="flex items-center gap-2"><Award className="w-6 h-6 text-slate-400" /><CardTitle className="text-xl">Granite Stake</CardTitle></div><CardDescription className="text-zinc-300">Stake {details.amount} {details.asset}</CardDescription></CardHeader>
        <CardContent className="space-y-6">
          <div className="bg-slate-500/10 border border-slate-500/20 rounded-lg p-6 text-center"><span className="text-sm text-zinc-400 block mb-2">Stake Amount</span><p className="text-4xl font-bold text-slate-400">{details.amount}</p><p className="text-xs text-zinc-500 mt-2">{details.asset}</p></div>
          {details.stakingAPY && <div className="bg-zinc-900/50 p-4 rounded-lg border border-slate-500/20"><div className="flex items-center justify-between text-sm"><span className="text-zinc-400">Staking APY:</span><span className="text-green-400 font-mono">{details.stakingAPY}</span></div></div>}
